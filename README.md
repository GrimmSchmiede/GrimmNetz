...
- **Docker als Install-/Betriebs-Grundlage:** Gameserver über Docker statt direkt per SteamCMD/systemd laufen lassen - würde Distro-Kompatibilitätsprobleme weitgehend beseitigen (immer dieselbe Container-Umgebung) und automatische Updates über einen Watchtower-artigen Mechanismus ermöglichen (aktuell gibt es dafür gar keine Lösung, nur Neuinstallation). Größerer, eigenständiger Umbau - betrifft Provisionierung, Firewall-Handling, Konsolen-Zugriff und jedes Spiele-Template gleichzeitig, kein Nebenbei-Feature
